...
> **Status: Stages A–C implemented.** Format spec v0.1 is frozen (`docs/SPEC.md`); the decision records are `accepted`. The core CLI, Bitcoin anchoring, and the Claude Code hook adapter are built and tested (81 behavioral tests, all through the public CLI). Currently in local dogfooding before any wider claim is made.
...
- **Next**: dogfooding — running the hook on real sessions and letting usage decide what earns building next.
