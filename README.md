...
Inside is a manifest, written last, listing every chain by its head and every other file by its hash. That is what makes one command enough: `verify-package` walks each chain, checks each file against the manifest, judges each seal, and prints the verdict on the last line, the way `verify` does.

`SELF-CONSISTENT` means every chain walks clean and every file matches the manifest. On its own that is worth what it says and no more, and the verdict prints its own limit: a wholesale regeneration would look the same. Two optional seals raise it, one putting the manifest's hash in a Bitcoin block so the package can be shown to have existed by then, one signing the manifest with an SSH key you hold so the recipient can tell which key packed it. [docs/PACKAGE.md](docs/PACKAGE.md) has both, and the words each verdict uses.
...
What no verdict says is that the record is true. A package confirms it is unaltered since it was packed, when it existed, and which key packed it. Whether the receipts describe what the agent really did is the chain's business, and the chain is caged the same way.
...
| Regenerating the whole chain from scratch | tiered | `verify --expect-head` against a head you kept, a head published off the machine at every session end, or an anchor: a regenerated chain can only carry young anchors |
