...
def write_package(unit, sessions, drawer, report, stage, packed, seals):
    ...
    manifest last, declaring `seals` before any of them exists so a
    stripped seal is caught (seal_package applies them). `sessions` is
    {session: [chains]} in the package's order; `drawer` is the one
    whose project record ships. Returns the file names in the order
    they were written, which is the order the zip keeps."""
