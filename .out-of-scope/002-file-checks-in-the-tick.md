...
> The supervisor's scan never passes `--files` to `loxodonta verify`. The tick judges chains; file divergence is a question the operator asks deliberately, per chain, with `loxodonta verify --files`.
