...
- `supervisor package <session-id | entry-address>` (ADR-0026, #176): one session as a package, siblings included: every chain and its anchor sidecar, the drawer's project record, `witness.json` (the completeness row and the scan's verdicts, labelled testimony in the file), a plain-words README, and `manifest.json` written last, format `loxodonta-package/1`, chains listed by head and entry count, every other file by sha256 and byte count, `"seals": []` declared. A zip by default, `--folder` for an unpacked folder. `--transcript`, `--anchor`, and `--sign` are later slices.
