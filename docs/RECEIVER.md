...
`--new-token` retires a URL that leaked: the receiver prints a new one and the old path answers 404 from the next start on. Rewire the sender with the new URL; the files on disk are untouched. To the sender a new URL is a new remote, so each chain goes again from its genesis (section 4, *where it resumes*) and the append rule drops every line of it this receiver already holds; the keeper also sends the current head once more, which lands in `heads.jsonl` as a second line for the same head.
