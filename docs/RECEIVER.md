...
```
receiver 0.7.0 keeping /home/op/.loxodonta/receiver
...
`--new-token` retires a URL that leaked: the receiver prints a new one and the old path answers 404 from the next start on. Rewire the sender with the new URL; the files on disk are untouched.
...
**The sender's side.** The recorder speaks this contract from `publish --chain --log LOG URL` and from a wired session end (`install-hook --profile custom --publish-chain URL`, docs/HOOK.md); `supervisor scan|serve --publish-every AGE --publish-chain URL` sends by running that same command, so there is one sender and not three.

- **Where it resumes.** The memo beside the chain, `<log>.published.jsonl`, holds one row of kind `chain` per acknowledged batch — `first`, `last`, the head after `last`, the time, the event — and the next send starts at the entry after the largest `last` it holds, or at genesis when it holds none. That row is appended on a `2xx` and on nothing else, so the cursor never passes an entry the receiver did not acknowledge. A memo that was lost or never written sends from genesis again, the append rule's first clause drops the duplicates, and the file on the receiver is unchanged.
- **How it batches.** Lines go in the order they sit on disk while the body stays under the 8 MiB cap; a longer tail goes in several batches, each its own POST, each acknowledged and written down before the next begins. A single line past the cap goes alone, and the receiver answers for it.
...
- **What it spends.** At session end the whole send runs under the head publish's budget, three seconds on Claude Code: each batch waits at most that long, name lookup included, and no batch begins once the budget is spent. Whatever did not fit is the keeper's, on its next turn, from the same cursor. (Codex does not get the flag at all: three seconds for the whole SessionEnd hook, half of it already the head's.)
...
Wherever the chain is published — `--publish-chain`, and the `full` tier that will fold it under one word — every entry crosses the network: the timestamp, the actor, the action line, the file references. Action lines are command lines and can carry anything the agent typed. Without `--cert` and `--key` they cross in the clear, and the startup line says so in as many words: *speaking plain HTTP*. That is acceptable on a wire the operator owns end to end (a LAN, an SSH tunnel, a VPN) and nowhere else.
