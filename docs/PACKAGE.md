...
Assembly follows ADR-0007's write order, and `supervisor package` honors it or the package is unverifiable: the chain snapshot and sidecars first, each session's transcript beside its chains, then `project.json`, then `witness.json`, then `README.md`, then `manifest.json`, then the seals, the anchor before the signature. The zip keeps that member order: the manifest comes after everything it lists, and the seal files follow the manifest in the declared order: the anchor sidecar, then the signature, then the public key.
