...
`--authority-chain FILE` is the certificate chain (PEM) the recipient saved from the authority, and it is what every token in the package is judged against — the manifest's, with `--stamp`, and any a chain carries. Without it a token is *present and not judged*, a note and never a verdict: the recipient is never told a token passed when nobody checked it. The package does not carry the chain file and must not (§2).
...
NO-STAMPS: loxodonta-package-b5d1e0a7-3c62-4f89-a0d4-8e21f6b4c907/receipts-b5d1e0a7-3c62-4f89-a0d4-8e21f6b4c907.jsonl.stamps.jsonl not found — the authority timestamp is optional; run `loxodonta stamp --authority URL` to add one
VALID
...
```
seal stamp: STAMPED: https://freetsa.org/tsr signed this manifest's sha256 — openssl accepted the token against tsa.crt; the time inside it is the authority's word, not this machine's (`openssl ts -reply -text` prints it)
residual trust: this package is unaltered since it was packed, and it existed by the time inside that token if the authority's clock and key custody are what the authority says. That the record inside is true and complete, and which key packed it, rests on the issuer's word alone, since no anchor is declared.
SELF-CONSISTENT + STAMPED: every chain walks clean and every artifact matches the manifest, and the authority the manifest names signed its sha256 under that authority's own clock; indistinguishable from a wholesale regeneration, since no anchor is declared
```

Exit 0 — and read that last clause, which is the difference between this rung and the anchor's. A regenerated package stamped this afternoon produces the same line, because an authority will stamp whatever it is handed, and the one that stamped this one could be asked for a different time. The token says *when* on a party's word; the anchor says it on nobody's, which is why the pair is `--anchor --stamp` and the answer to "which do I need" is usually both. Without `--authority-chain` the seal line reads `seal stamp: not judged: no --authority-chain FILE given — the rung is neither earned nor failed; ...`, the last line stays `SELF-CONSISTENT` with *its authority timestamp was not judged* added, and the exit code is whatever the rest of the ladder says; the same line appears on a machine with no `openssl`. Delete the sidecar and the last line reads `SEAL-MISSING`, exit 3; put a token over another digest in its place, or flip a bit inside the one that is there, and it reads `SEAL-INVALID`, exit 3. A chain inside that carries its own session-end token prints `STAMPED: entries 0..N existed when <authority> signed this head` under that chain and changes the package verdict in none of those cases — unless that token does not hold, which is the chain's own `STAMP-INVALID`, exit 3, named for the mechanism that failed rather than borrowing the anchor's word.
...
- **What the token says, and who could unsay it.** That the authority the manifest names signed this manifest's sha256, under that authority's clock. The authority is a party, and a party can be leaned on: a colluding or compromised authority stamps any time it is asked for, past or future, and nothing in the package would look different. A token is trusted exactly as far as that authority's honesty and key custody, which is why it never stands where the anchor stands (ADR-0032). It is also judged against a certificate chain that expires and can be revoked: re-stamping before expiry and keeping the chain file are the operator's habit and are not built, so a recipient who loses the chain file holds a token nobody can judge. Ten-year evidence is the anchor's job.
