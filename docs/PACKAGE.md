...
```
supervisor package <session-id | entry-address> [--out PATH] [--folder] [--witness DIR] [--transcript] [--anchor [--calendar URL]...] [--stamp URL] [--sign KEYFILE]
supervisor package [--repo PATH]                 [--out PATH] [--folder] [--witness DIR] [--transcript] [--anchor [--calendar URL]...] [--stamp URL] [--sign KEYFILE]
```
...
Assembly follows ADR-0007's write order, and `supervisor package` honors it or the package is unverifiable: the chain snapshot and its sidecars first (the anchor sidecar and the stamps sidecar both, when the chain has them), each session's transcript beside its chains, then `project.json`, then `witness.json`, then `README.md`, then `manifest.json`, then the seals, the signature first, then the authority timestamp, then the anchor: signing is the step that can fail on a passphrase or a touch, and the other two are the ones that leave the machine, so a signing that fails costs neither; between those two the quick round trip goes before the slow one, the order the session end uses (ADR-0032 ruling 3). The zip keeps that member order: the manifest comes after everything it lists, and the seal files follow the manifest in the order they were applied: the signature, then the public key, then the stamps sidecar, then the anchor sidecar.
...
**The third seal, the authority timestamp.** `--stamp URL` applies it (ADR-0032, through ADR-0026 ruling 4), alone or beside either of the others. Once the manifest is written, and after the signature and before the anchor when they are asked for too, the supervisor drives `loxodonta stamp --manifest <manifest> --authority URL`, which asks that RFC 3161 timestamp authority for a token over the manifest's sha256 and writes the reply beside it as `manifest.json.stamps.jsonl`: an ordinary stamp record (docs/ANCHORING.md §6) whose `head` is the manifest's digest and which has no `n`, since a manifest has no entries. The supervisor never speaks RFC 3161 itself. The manifest declares `"seals": ["stamp"]` before it is written, so a package stripped of its token is `SEAL-MISSING` (§5), never a quiet downgrade. What leaves the machine is the manifest's 32-byte digest, to the authority the operator named, from the operator's address, and only with the flag. There is no default authority: whom to trust is the whole choice, and a default would make it for you. An authority that grants nothing leaves no package behind, zip or folder, for the anchor's reason. **This is not an anchor and the package never calls it one**: an anchor's proof is nobody's product and replays offline, where a token is somebody's signed word, trusted exactly as far as that authority's certificate and its honesty about its own clock (GLOSSARY: *Authority timestamp*). The two sit side by side, and `--stamp` beside `--anchor` is the pair ADR-0032 intends. One thing the package cannot carry is the certificate chain the token is judged against: a chain shipped by the issuer is the issuer's word about whom to trust, so the recipient saves it from the authority themselves, or receives it by a route this package cannot rewrite, and passes it as `verify-package --authority-chain FILE` (§5). The supervisor says so when it prints `stamped:`.
...
| `receipts-<session>.jsonl.stamps.jsonl` | a chain's stamps sidecar, when the drawer has one (docs/ANCHORING.md §6): the tokens an authority signed over that chain's heads, kept verbatim | sha256 and byte count |
...
| `manifest.json.stamps.jsonl` | the manifest's authority timestamp, with `--stamp` (§2): one record, `head` the manifest's sha256, no `n`, the authority's whole reply kept verbatim in base64 | nothing: a seal is applied after the manifest is written, which declares it (`seals`) and cannot list it |
...
| `chains` | one row per chain, in the package: `path`, `head` (the last entry's `entry_hash`), `entries` (line count), `anchors` (the sidecar's file name, or `null`), `stamps` (the stamps sidecar's file name, or `null`), and `transcript` (the packaged transcript's file name) only when one travels with the chain's session, every chain of that session naming the same file. The verifier refuses a `transcript` that is not a bare file name or that `artifacts` does not list. |
...
| `seals` | the seals the package should carry: `[]` unsealed, `["anchor"]` with `--anchor`, `["stamp"]` with `--stamp`, `["signature"]` with `--sign`, and the declared list in that order with any combination of them (the order they are judged: the two *when* seals before the *who*, and the anchor before the token, since an anchor's proof is nobody's product). A declared seal that is absent is `SEAL-MISSING`, never a silent downgrade (ADR-0007). |
...
```
loxodonta verify-package PATH [--authority-chain FILE]   # a zip, or an unpacked folder
```

`--authority-chain FILE` is the certificate chain (PEM) the recipient saved from the authority, and it is what every token in the package is judged against — the manifest's, with `--stamp`, and any a chain carries. It is one file for all of them, so when the stamps records name more than one authority (a chain stamped at session end by one, the manifest stamped at packaging by another) it is one chain file holding every authority's certificates (concatenated PEM). Given one authority's chain alone, the other's tokens fail against it and read `STAMP-INVALID` or `SEAL-INVALID`, exit 3, because a token that does not verify against the file you trust is not evidence to you. Without the flag a token is *present and not judged*, a note and never a verdict: the recipient is never told a token passed when nobody checked it. The package does not carry the chain file and must not (§2). The names in the stamps records are the packer's note of whom it asked, in files the manifest does not list; the verifier never presents one as the signer.
...
2. **Each chain**, headed `chain: <name> (manifest: head <12 hex>…, N entries)`, followed by the recorder's own `verify --anchors` output verbatim, with `--stamps` added when the manifest's row for that chain lists its stamps sidecar. Anchor lines appear here as detail: `ANCHORED`, `ANCHOR-PENDING`, `NO-ANCHORS`, or a mismatch; so do the chain's own authority timestamps when they are listed, `STAMPED`, `stamp not judged`, or `STAMP-INVALID` (docs/ANCHORING.md §6). Both are detail for the same reason: they commit a chain head, and the package's own rungs are the manifest's seals and no others (ADR-0026 ruling 6). A stamps sidecar the manifest does not list is judged by nobody: it prints as `unlisted` with the other files nothing vouches for, and a listed one that was taken away is `ARTIFACT-DIVERGED`. When the manifest names a transcript on the chain, the recorder judges it the way `verify --transcript PATH` does, and those lines are part of the same output: `COMMITMENT HOLDS (entry N: first B bytes)` or `COMMITMENT DIVERGED (entry N)` per commitment, oldest first; then `transcript tail: B bytes after the last commitment (entry N), uncommitted by the chain`, the bytes only the manifest vouches for; and `TRANSCRIPT-DIVERGED` when a committed prefix differs or the transcript is shorter than a commitment. A chain that holds commitments when no transcript travelled gets `TRANSCRIPT-UNRESOLVED: N transcript commitment(s) in this chain, no transcript in this package — commitments unjudgeable; chain verdict unaffected`, a note and never a verdict (ADR-0017). Then the walked head and line count are compared with the manifest's row; a difference is reported as `<name>: off the manifest`.
...
5. **Each declared seal.** The anchor is judged offline the way `verify --anchors` judges a chain's (docs/ANCHORING.md §3): every record of `manifest.json.anchors.jsonl` must name this manifest's sha256 and replay. A completed proof prints `seal anchor: ANCHORED: the manifest existed by Bitcoin block H — confirm merkle root R against a block source you trust`; a pending one prints `seal anchor: ANCHOR-PENDING: the manifest was submitted <ts> via <calendar> — run ...` with the upgrade command, and the rung waits; a calendar that never came back when another one settled the same manifest prints `seal anchor: ANCHOR-UNANSWERED: ...` and the rung is earned, since the anchor's claim is about the digest and not about any one calendar (#199); a record for another digest, or one whose proof does not replay, prints `seal anchor: SEAL-INVALID: <why>`; a declared anchor with no sidecar, or an empty one, prints `seal anchor: SEAL-MISSING: ...`. The authority timestamp is judged by `openssl ts -verify`, never by the recorder itself (ADR-0032 ruling 5, the delegation ADR-0026 settled for the signature): each record of `manifest.json.stamps.jsonl` must be a token over this manifest's sha256, and openssl must accept it against `--authority-chain`. A token that holds prints `seal stamp: STAMPED: a key certified by <file> signed this manifest's sha256 under its own clock (the record names <URL>, testimony) — the time inside the token is that key's word, not this machine's`, and `openssl ts -reply -text` prints that time for a recipient who wants to read it. That is what openssl checked and all it checked: the URL comes from the stamps record, a file the manifest does not list and the writer can edit, so it is named as testimony and never as the signer, the rule ADR-0008 ruling 4 set for the signature. Whose key it is rests on where the recipient got the chain file; a token over another digest, or one openssl rejects, prints `seal stamp: SEAL-INVALID: <why>`; a declared token with no sidecar, or an empty one, prints `seal stamp: SEAL-MISSING: ...`. A recipient with no openssl, or who named no chain file, sees `seal stamp: not judged: <why> — the rung is neither earned nor failed; ...`. The signature is judged by ssh-keygen, never by the recorder itself: the shipped `manifest.json.pub` becomes a one-line allowed-signers file under the principal `issuer`, and `ssh-keygen -Y verify -n loxodonta-package` says whether `manifest.json.sig` is that key's signature over the manifest's exact bytes. A signature that holds prints `seal signature: SIGNED (key: SHA256:<fingerprint>): the manifest, and transitively every artifact it lists, was issued by the holder of that key and has not changed since signing — compare the fingerprint against a channel this package cannot rewrite`, the fingerprint from `ssh-keygen -lf` on the shipped key and never a name or the key's comment (ADR-0008 ruling 4); one that does not hold, because the manifest changed after signing or the shipped key is not the one that signed, prints `seal signature: SEAL-INVALID: <ssh-keygen's words> — ...`; a declared signature with no `.sig` or no `.pub` prints `seal signature: SEAL-MISSING: <which> not in this package — ...`. A recipient whose ssh-keygen is missing, cannot run, or predates `-Y verify` (OpenSSH 8.0) sees `seal signature: not judged: <which> — the rung is neither earned nor failed; ...`, and the ladder reports the rungs it could judge (ADR-0026 ruling 6). The seal files are judged here as seals, never named as unlisted. A kind this verifier does not know is named as declared and not judged.
6. When the ladder allows it, one line of **residual trust**; then **the package verdict**, the last line, in ADR-0007's words, with its rungs in ADR-0007's order: `+ ANCHORED` when the manifest's anchor completed, then `+ STAMPED` when openssl accepted its token, then `+ SIGNED (key: SHA256:...)` when its signature holds (`SELF-CONSISTENT + ANCHORED + STAMPED + SIGNED (key: SHA256:...): ..., and the manifest existed by Bitcoin block H, and a key certified by the --authority-chain file signed the manifest's sha256 under its own clock, and the manifest, and transitively every artifact it lists, was issued by the holder of key SHA256:... and has not changed since signing`), so a script reads the last line as it does for `verify`. The residual-trust line names what each seal gives, when and which key, each with the recipient's own check attached (the merkle root against a block source; the time inside the token, *if the holder of that key keeps an honest clock and sole custody of the key*, whose key it is resting on where the chain file came from; the fingerprint against a published one), and what still rests on the issuer's word: that the record inside is true and complete, in every case; that it existed before today, without a completed anchor and without a token; which key packed it, without a signature that holds. A token answers *when* on the authority's word, which is why it never removes the anchor's line and never reads like it: with a token and no anchor the limit still says *indistinguishable from a wholesale regeneration, since no anchor is declared*, because an authority can be asked for a token over anything, today included.
...
- `SELF-CONSISTENT + STAMPED`: openssl accepted the manifest's token against the certificate chain the recipient named. It means that a key that chain certifies signed this manifest's sha256 under its own clock, and nothing more: the manifest names no authority, and the name in the stamps record is the packer's testimony, so the rung never says who signed; whose key it is, the recipient knows from where they got the chain file. The time inside the token is that key holder's word, an authority that lies about its clock can stamp any time it likes, and a token is only as good as that authority's key custody. So it is never *anchored* and never *proof*, it never replaces the anchor's rung, and with no anchor the limit still names a wholesale regeneration (ADR-0032 rulings 1 and 5). The rung is the manifest's token and no other's: a stamped chain inside prints `STAMPED` under that chain as detail and earns the package nothing. After `+ ANCHORED`, before `+ SIGNED`.
...
- `STAMP-INVALID`: an authority timestamp packaged with a chain is not evidence for that chain: its head is nowhere in the chain, or openssl rejected the token, or the token is over some other head; the chain's own `STAMP-INVALID` line above says which. Not the stamped history. (The word is the chain's; the manifest's own token fails as `SEAL-INVALID`, since that one is a seal.)
- `SEAL-MISSING`: the manifest declares a seal and the package carries no file for it: no anchor sidecar, or an empty one; no `manifest.json.stamps.jsonl`, or an empty one; no `manifest.json.sig`, or no `manifest.json.pub`. A package stripped of a seal fails rather than reading as merely unsealed (ADR-0007's declared seal set).
- `SEAL-INVALID`: the package carries a seal that is not this manifest's: an anchor record whose `head` is another digest or whose proof does not replay; a token over another digest, or one openssl rejects; a signature ssh-keygen rejects, because the manifest changed after signing or the shipped key is not the one that signed; a `.pub` that is not a key ssh-keygen reads. Evidence that does not verify is not evidence.
...
|---|---|---|
| 0 | `SELF-CONSISTENT`, with or without `+ ANCHORED`, `+ STAMPED` and `+ SIGNED (key: ...)`; a declared signature or token this machine could not judge changes nothing here | `VALID` |
| 1 | `CHAIN-BROKEN` | `BROKEN` |
| 2 | `ARTIFACT-DIVERGED` | `FILES-DIVERGED`, its package sibling per ADR-0007 |
| 3 | `SEAL-INVALID`, `SEAL-MISSING`, a chain's `ANCHOR-MISMATCH` or `STAMP-INVALID` | `HEAD-MISMATCH`: not what was issued |
...
### The same package, stamped

Packaged with `--stamp https://freetsa.org/tsr`, the summary says `seals: stamp`, the manifest declares `"seals": ["stamp"]`, `manifest.json.stamps.jsonl` travels after it, and the supervisor ends with `stamped: the manifest's sha256 went to <authority>; send the recipient that authority's certificate chain by another route ...`. The recipient saves that chain from the authority and names it:

```
seal stamp: STAMPED: a key certified by tsa.crt signed this manifest's sha256 under its own clock (the record names https://freetsa.org/tsr, testimony) — the time inside the token is that key's word, not this machine's (`openssl ts -reply -text` prints it)
residual trust: this package is unaltered since it was packed, and it existed by the time inside that token if the holder of that key keeps an honest clock and sole custody of the key; whose key it is rests on where you got the chain file, never on the name the stamps record gives. That the record inside is true and complete, and which key packed it, rests on the issuer's word alone, since no anchor is declared.
SELF-CONSISTENT + STAMPED: every chain walks clean and every artifact matches the manifest, and a key certified by the --authority-chain file signed the manifest's sha256 under its own clock; indistinguishable from a wholesale regeneration, since no anchor is declared
```

Exit 0 — and read the rung first: it says a key your chain file certifies signed, not that FreeTSA did. The URL on the seal line is the packer's note of whom it asked, and a writer can change it without touching a seal; the name that counts is the one on the chain file you saved. Then read that last clause, which is the difference between this rung and the anchor's. A regenerated package stamped this afternoon produces the same line, because an authority will stamp whatever it is handed, and the one that stamped this one could be asked for a different time. The token says *when* on a party's word; the anchor says it on nobody's, which is why the pair is `--anchor --stamp` and the answer to "which do I need" is usually both. Without `--authority-chain` the seal line reads `seal stamp: not judged: no --authority-chain FILE given — the rung is neither earned nor failed; ...`, the last line stays `SELF-CONSISTENT` with *its authority timestamp was not judged* added, and the exit code is whatever the rest of the ladder says; the same line appears on a machine with no `openssl`. Delete the sidecar and the last line reads `SEAL-MISSING`, exit 3; put a token over another digest in its place, or flip a bit inside the one that is there, and it reads `SEAL-INVALID`, exit 3. A chain inside that carries its own session-end token, listed in the manifest, prints `STAMPED: entries 0..N existed when a key certified by <file> signed this head under its own clock (the record names <URL>, testimony)` under that chain and changes the package verdict in none of those cases — unless that token does not hold, which is the chain's own `STAMP-INVALID`, exit 3, named for the mechanism that failed rather than borrowing the anchor's word.
...
- **What the token says, and who could unsay it.** That a key the recipient's chain file certifies signed this manifest's sha256, under that key holder's clock. Not which authority: the package names none it can vouch for, and the name in its stamps record is the packer's testimony. The authority is a party, and a party can be leaned on: a colluding or compromised authority stamps any time it is asked for, past or future, and nothing in the package would look different. A token is trusted exactly as far as that authority's honesty and key custody, which is why it never stands where the anchor stands (ADR-0032). It is also judged against a certificate chain that expires and can be revoked: re-stamping before expiry and keeping the chain file are the operator's habit and are not built, so a recipient who loses the chain file holds a token nobody can judge. And openssl checks the authority's certificate as of the moment it verifies, not the moment the token was issued, so a token whose certificate has expired since may read `SEAL-INVALID` or `STAMP-INVALID` rather than *not judged*: the token did not change, the certificate's life ran out. Long-term validation, which would judge a token as of its own time, is not built (ADR-0032). Ten-year evidence is the anchor's job.
