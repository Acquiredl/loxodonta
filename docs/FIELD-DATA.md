...
|---|---|---|---|---|
| 2026-09-03 | none (the author's own dry run; not filed) | Windows | 38 | Two things. First, the export read a session's verdict from its last sibling, so the one session whose first chain broke and whose recording moved to `-002` exported as VALID; fixed in PR #111 (the worst sibling speaks for the session), together with `matchers: null` from misreading the baseline. Second, 24 of 38 sessions were ENDED-DEFICIT. Walking two transcripts against their chains: the missing receipts were Read, Grep, PowerShell and MCP calls from before coverage went wide on 2026-08-31, judged by today's `*` because the calibration memory's first epoch extends over all time before it (the witness over-counts). Ruled 2026-09-11 by ADR-0029, from issue #114: the first observation now stamps itself, a session older than it reads `BEFORE-MEMORY` and is judged not at all, and the export carries the count of such sessions instead of a wall of deficit rows. On the tools the old matcher did cover, chain and transcript agreed to within a handful, and that handful was writes to files on another drive, the hook bug fixed 2026-08-30 (f2c1740). After the widening, chain and transcript agree tool for tool. The three ENDED-SURPLUS sessions are receipts from subagent tool calls (WebFetch and WebSearch, mostly), which the harness fires under the parent session id but wrote to a separate transcript the witness did not read. Fixed 2026-09-11 (#211): the witness reads `<session>/subagents/agent-*.jsonl` beside the parent. Every surplus in the author's store closed, four of the seven landing exactly (the largest at 3721 against 3721), and the three that did not are all from before the cross-drive hook bug was fixed on 2026-08-30. |
