...
- **Anchor at session end, if you opted in (ADR-0024).** `install-hook --anchor-at-session-end` writes `--anchor` onto the wired SessionEnd command, and from then on every session end anchors the chain head: after the tail commitment, so the head anchored is the sealed one; under a twelve-second budget inside the harness's twenty-second SessionEnd timeout; a sidecar record only for calendars that answered; nothing printed on failure, since an exit hook that complains is noise nobody can act on, and staleness is the supervisor's to surface. The budget left over goes to upgrading that drawer's pending proofs, oldest first, one request each, so a machine that never runs the supervisor still reaches Bitcoin-grade, one session late. The install command states the choice each time: a re-run without the flag turns it off and says so; `uninstall-hook` removes it with the rest. What leaves the machine is a 32-byte digest of the head, to the public calendars, from your address, only after you opted in. Codex does not get the flag: its SessionEnd hook is capped at three seconds, too short to reach a calendar with margin, so the installer refuses it with a note, and a Codex operator uses the supervisor's `--anchor-every`. What none of this survives: the agent killing the process, where the kill is the last receipt and the silence is the supervisor's alarm.
- **Publish the head at session end, if you opted in (ADR-0025).** `install-hook --publish-head URL` writes `--publish "URL"` onto the wired SessionEnd command, and from then on every session end POSTs the chain head to that URL: after the tail commitment, so the head that leaves is the sealed one, and before the anchor, so a slow calendar can never cost the one POST; the anchor takes what is left of the twelve-second budget. What leaves is one JSON body with `head`, `n`, `session`, `ts`, `event`, and one readable line under both `text` and `content` (Slack and Teams render one key, Discord the other); nothing else, no path, no project name, no action line, no chain bytes. The POST waits at most three seconds, name lookup included, and is then left behind; a redirect is a failure, so the head goes to the URL you wrote or nowhere; nothing is printed and the hook never fails over it. The URL must be plain http or https with nothing a shell could act on, because the wired command runs through a shell: the installer refuses anything else. Which remotes count as a head record, and why a gist under your login does not, is ADR-0025 and GLOSSARY *Head record*; the check a published head enables is `verify --expect-head` with a head copied from the channel. Like the anchor flag, the install command states the choice each time, a re-run without the flag turns it off and says so, and `uninstall-hook` removes it with the rest. Codex gets the flag too, with a tighter bound: its whole SessionEnd hook is capped at three seconds, so a Codex hook waits 1.5 seconds for the POST rather than three (the measurement is below, under [Codex CLI](#codex-cli)). A session that never reaches its end (the bad day: the hook was stripped, so no session end fired) is the supervisor's keeper's to cover: `supervisor scan|serve --publish-every AGE --publish-url URL` publishes an aged head once, through `loxodonta publish`, and the scan report says per chain when a head last left the machine (docs/TOUR-SUPERVISOR.md §2).
- **Each session-end step writes down how it went (#240).** The anchor and the head publish are quiet on failure by design, and quiet at the moment is not the same as silent in the record: after each step the hook appends one row of kind `attempt` to the sidecar that step already owns, the anchors sidecar (`<log>.anchors.jsonl`) for the anchor and the publish memo (`<log>.published.jsonl`) for the head. The row carries the step (`anchor`, `publish-head`), the time, the budget in seconds the step had, and the outcome: `submitted` or `sent`, or the one line the step produced, such as `no answer within 3 seconds`, `the remote answered 404`, or `no calendar answered within 12 seconds`. Never the URL, which is a credential: the outcome line is what the socket said or the exception's bare name, so a TLS certificate-name mismatch surfaces the public hostname inside the reason text, the host and never the secret. A head already anchored at session end leaves no row, since the anchor step did not run. The row is testimony and never a proof: `verify --anchors`, the supervisor's keeper and `verify-package` skip it by its kind, so a sidecar holding only attempt rows reads as an empty one. What reads it is the supervisor: `scan --json` carries per chain `left` (when a head last left the machine by any route) and `last_failed` (the last step that failed: step, time, outcome), cadence or no cadence; the dashboard's project tile shows both; and when the wired SessionEnd command carries `--publish` and no chain holds a sent head, the report says so in one sentence under `published`. The chain's schema is untouched: a note about network luck is not part of the session's history.
...
`install-hook` also writes down what it wired, in `~/.loxodonta/coverage.json`: one appended entry per change, carrying `since`, `matchers` and the `harness` whose settings file it touched (ADR-0030). The supervisor reads that file to date coverage from when it began rather than from its own first look, which matters because the documented install order puts a week of work between the two. It is testimony, trusted for nothing beyond calibration, and merged in memory rather than copied into the supervisor's baseline. `uninstall-hook` writes nothing: a start claim says more calls owe receipts, an end claim says fewer, and "nothing was owed from here" is the one sentence that could retire the completeness alarm.
...
receipts verify --log ... --expect-head <head from your channel>  # against a published head (ADR-0025)
```
