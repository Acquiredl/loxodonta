...
- **Publish the head at session end, if you opted in (ADR-0025).** `install-hook --publish-head URL` writes `--publish "URL"` onto the wired SessionEnd command, and from then on every session end POSTs the chain head to that URL: after the tail commitment, so the head that leaves is the sealed one, and before the anchor, so a slow calendar can never cost the one POST; the anchor takes what is left of the twelve-second budget. Pick a remote the credentials on this machine cannot delete from. A chat incoming webhook (Slack, Discord, Teams) posts and can do nothing else; a retention-locked bucket takes puts and no deletes; a gist under your `gh` login is not one, because the token deletes it (the head-record test, GLOSSARY *Head record*). What leaves the machine is one JSON body with exactly `head`, `n`, `session`, `ts`, `event` (`session-end`), and one human-readable line repeating those under both `text` and `content`, so Slack and Teams render one key, Discord the other, and a JSON receiver reads the fields. Nothing else: no path, no project name, no action line, no chain bytes. The POST waits at most three seconds, prints nothing on failure, and never fails the hook: a slow or unreachable URL costs nothing else, and staleness is the supervisor's to surface. The check it enables is `verify --expect-head` with a head copied from the channel: every published head must be an entry hash inside its chain, so a regenerated chain fails on the earlier heads and a fake head the agent posts matches nothing. Like the anchor flag, the install command states the choice each time, a re-run without the flag turns it off and says so, and `uninstall-hook` removes it with the rest. Codex does not get the flag yet: its SessionEnd hook is capped at three seconds, and whether one POST fits inside that is measured before Codex gets it, so the installer refuses with a note. The webhook URL sits in your settings file, readable by the agent: that buys it the right to post fakes, which match nothing, and to stop posting by killing the hook, which the completeness alarm watches; it buys no deletion, or the remote was never a head record.
