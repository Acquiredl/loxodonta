...
- **No SessionStart digest.** Codex injects hook output into context only through a JSON envelope the digest does not emit. Codex reads the same memory over MCP instead — [docs/MCP.md](MCP.md).
