...
- **Coverage is every tool call that runs, completed or failed.** The shipped matcher is `*` (ADR-0016), on `PostToolUse` and, since #239, on `PostToolUseFailure` beside it: reads, searches, fetches, spawns, and MCP tools all leave a receipt, not just state changes. What a failed call owes the completeness alarm is ADR-0034's to say. A sensor with an allowlist has blind spots an attacker can enumerate by reading this public repo.
