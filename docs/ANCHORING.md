...
- `kind` — `stamp`: what the row is (ADR-0038). The first tokens ever written name no kind, and a row with no `kind` reads as a stamp, whenever it was written.
...
A row of any other kind, or of a kind that belongs in another sidecar (an `anchor`, or the memo's `head`), is named by its line and never judged: `STAMP-UNKNOWN-KIND: line 3 of receipts.jsonl.stamps.jsonl is of kind "witness-note"`, earning nothing and leaving the exit code as it was. It holds no token, so a head it names is still asked about. A line that is not a JSON object is still `STAMP-INVALID`.
...
4. A row that is not a stamp is not judged by any of the above. An attempt row is skipped silently. A row of a kind this verifier does not know, or of another sidecar's kind, is named first, as `STAMP-UNKNOWN-KIND: line N of <sidecar> is of kind "<kind>"`: it earns nothing and leaves the exit code as it was. In a package's `manifest.json.stamps.jsonl` the line is headed `seal stamp:`.
