...
3. No `--authority-chain` at all, a chain file that isn't there, or no `openssl` on the path: `stamp not judged: <reason>`. The token is present and nobody judged it, a note and never a verdict, and the exit code stays the chain's. This is the posture ADR-0026 set for the issuer signature and `ssh-keygen`, kept here for the same wall: the stdlib has no public-key cryptography, so either `openssl` checked the signature or nobody did, and the output says which.
