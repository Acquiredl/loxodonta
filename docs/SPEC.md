...
A sidecar's name may hold something that is not a regular file once links are followed: a folder, a pipe, a device, or a file the verifying user may not open. The writer can put one there with one command, so it is judged as a line that cannot be read, never as an absent sidecar and never as a reason to stop (#364). `verify --anchors` prints `ANCHOR-INVALID: <sidecar> cannot be read as a sidecar: <why>`, with `<sidecar>` the bare name and `<why>` `it is a folder, not a file` for a folder, `it is not a regular file` for a pipe or a device, or the system's reason for a file that will not open, and `verify --stamps` prints the same after `STAMP-INVALID`; each is exit 3, as an unreadable line is (§9.2). In a package, one beside a chain is that chain's finding, `ANCHOR-MISMATCH` or `STAMP-INVALID` in the package's words (§10.5), and the artifact it stands in for is `ARTIFACT-DIVERGED`; one beside the manifest, under a declared seal, is `SEAL-INVALID` (§10.6). A reader or a writer opens the name without waiting, since an ordinary open of a pipe waits for the other end, and asks what it opened before it reads or writes a byte. The recorder's verbs that append to one say why they could not and exit 73 (`anchor`, `stamp`, `publish`), or 66 for `anchor --upgrade`, which must read it first, and `publish --chain` exits 69 (§9.8); the session end passes it by quietly, as it does every failure. A folder and a pipe are answered alike.

A sidecar holds one JSON object per line, its lines split as §1 splits a log's: the bytes before each `\n`, a `\r` just before it read as part of the ending, and the bytes after the final `\n`, when there are any, a line too. Lines are numbered from 1. A writer appends each row as one complete line, keys sorted and separators compact as §4 rules 1 and 2 have it, every character past ASCII written as a `\u` escape, and `\n` alone ending it. It never rewrites or removes a line: the sidecar is append-only by convention, and nothing but that convention keeps it so. A reader takes a row in any key order and any spelling JSON allows. This verifier also reads `NaN`, `Infinity` and `-Infinity`, which JSON does not allow, as numbers, so a line holding one is not refused and its row is judged like any other; refusing them is tracked in #365. A row is never hashed, so §4 rule 4 does not bind it: the attempt row's `budget` is a number with a fraction.

A line that is not a JSON object is **unreadable**: it is not JSON, or it is empty, or it is JSON and not an object (an array, a string, a number), or it holds a byte that is not UTF-8, or the reader cannot take it apart, as when it holds an integer past Python's integer digit limit (4,300 digits) or nests deeper than its recursion allows. A sidecar has no torn tail: a final line cut short is unreadable like any other (§6 names the torn tail of a chain only). A key given twice in one row is not refused, as it is in an entry (§6 step 1) and in a manifest (§10.2): the reader keeps the last of the two.

### 9.2 What a row is (ADR-0038)

A row names its **kind** in a string member `kind`, and a writer MUST write it on every row it writes. The kinds each sidecar holds, its evidence kind first:
...
| `n` | integer | That entry's `n`. Absent from a manifest's row, which has no entries (ADR-0026 ruling 4). Its value is not read, since the verifier takes `n` from the chain, but when present it MUST be an integer, never a boolean (§9.4 step 2). |
| `ts` | string | When the row was written, in §2's form: for a pending proof, when the head was submitted; for a completed one, which `anchor --upgrade` and the session-end upgrade append as a new row, when it was upgraded. Testimony. It may be absent, and when present MUST be a string. |
| `calendar` | string | The calendar the proof came from. Testimony, printed, and what pairs a pending proof with its completion (§9.4). It may be absent, and when present MUST be a string. |
...
| `response` | string | The base64 of the authority's whole `TimeStampResp`, verbatim. Its status, and whether a token follows the status, are read as §9.7 says; the token inside it is read only by `openssl`. |
...
1. An unreadable line is `ANCHOR-INVALID`.
2. A row that is not the shape of an anchor row is `ANCHOR-INVALID`: a `head` or a `proof` that is absent or not a string, a `proof` that is not base64 read strictly (nothing outside the alphabet, the padding whole), a `calendar` or a `ts` that is present and not a string, or an `n` that is present and not an integer (a boolean is not one). The line names the field and the JSON type it holds, never its value: `ANCHOR-INVALID: record's head is an array, not a string`. The shape is judged before the head is compared, so a row naming a head the chain does not hold, with no proof, is `ANCHOR-INVALID` and not `ANCHOR-MISMATCH` (#348).
3. A `head` that is not the `entry_hash` of an entry of the chain is `ANCHOR-MISMATCH`: this log is not the anchored history, which is what a whole-chain regeneration looks like.
4. A `proof` that does not replay from `head` (§9.5) is `ANCHOR-INVALID`: evidence that does not verify is not evidence.
5. A proof that replays to a Bitcoin attestation is `ANCHORED` for entries `0..n`, where `n` is that of the entry whose `entry_hash` is `head`. The block it names is claimed, or checked when a header was given (§9.6). Each completed proof prints its own line.
6. A proof that replays to a pending attestation earns nothing and is no finding. When a completed proof in the sidecar has the same `head` and `calendar`, a `calendar` absent from both counting as the same, it is that submission's own completion, and the pending row is superseded and prints nothing. When another calendar completed that head, the pending row is `ANCHOR-UNANSWERED`, and no upgrade is owed: the anchor's claim is about the head, not about any one calendar (#199). Otherwise it is `ANCHOR-PENDING`.
...
4. The reply is read offline, with no tool, for its status and whether a token follows it, and for nothing inside the token (RFC 3161 §2.4.2). A `TimeStampResp` is a DER SEQUENCE whose first element is the PKIStatusInfo SEQUENCE, whose first element is the status, an INTEGER of one to four content bytes; the token, when present, is the next element, a SEQUENCE, read for its tag and length alone. Definite lengths only, and every length within the bytes; anything after the token is left to `openssl`. Bytes that are not this shape, a status other than granted (0) or granted with modifications (1), or a granted status with no token after it, are `STAMP-INVALID`, naming which (#370): RFC 3161 puts a token in every granted reply and in no other, so such a reply holds none, and saying a token is present when none is would be the verifier's own mistake. This holds with or without `openssl` and a chain file.
5. The token goes to `openssl` as §10.8 says, with `head` as the digest it must carry and FILE as the chain. Accepted, it is `STAMPED` for entries `0..n`, `n` taken from the chain as in §9.4: a key the chain file certifies signed that head under its own clock, the time inside the token is that key's word, and the row's `authority` is named as testimony. Refused, it is `STAMP-INVALID`, with `openssl`'s reason. When no tool or input could judge it, for a reason §10.8 lists, it is `stamp not judged: <reason>`: the token is present, and nobody judged it.
...
`stamp` and the session end count a head as already stamped only when a stamp row for it has a `response` that passes steps 3 and 4, read as the query reads a reply; the token is not judged. One rule, so a row `verify --stamps` calls `STAMP-INVALID` never stands `stamp` down, and the supervisor's scan counts a stamp row as a head that left by the same rule (#370). A row of the wrong shape, or a reply that was not granted, does not stop the head being asked about, and the supervisor's keeper runs `stamp` and leaves the question to it (#366).

**The limit.** A reply's status sits outside the token's signature, so a row naming a head with a granted status and any SEQUENCE after it, a reply forged in nine bytes or copied from another head's row, stops a new stamp of that head, and nothing offline can tell it from the authority's. Such a row never reads as a judged stamp: without `openssl` and the authority's chain file it is `stamp not judged`, and with them `STAMP-INVALID`, since no key the chain certifies signed that head.

### 9.8 The memo

The memo is bookkeeping, kept so that a head is posted once and a published chain resumes where its remote stopped, and for nothing else. No verifier judges it and no `verify` flag reads it. `supervisor package` never packs it, and one found in a package is unlisted (§10.5). Its rows are read by kind as §9.2 says, with `head` its evidence kind, so a kind-less row is a sent head. The recorder reads the `chain` rows as the cursor for one remote: the greatest integer `last` among the rows whose `remote_id` is that remote's, or none, and then the send starts at genesis (docs/RECEIVER.md). A line of the memo that is not JSON is read past, which costs at worst a batch sent again and dropped by the receiver as a duplicate; a line holding a byte that is not UTF-8 stops the send rather than be guessed at, and so does a memo that is not a file (§9.1): `publish --chain` then says the memo could not be read and exits 69, and sends nothing. The supervisor's keeper reads the head rows and the attempt rows, to say when a head last left the machine and which step last failed (docs/TOUR-SUPERVISOR.md).

The conformance vectors hold the anchors and stamps sidecars: [`tests/vectors/`](../tests/vectors/README.md) has a pending proof, a completed one with and without its block's header, a mismatch, a proof that does not replay, an unreadable line, an attempt row alone, a kind-less row, `{"kind": null}`, an unknown kind and a misplaced `chain` row; and, needing no `openssl`, a kind-less stamp row whose granted reply holds a stand-in token, a rejection reply, an attempt row and an unknown kind, each with its exit and every line it prints.
...
A **bare name** is a string of at most 255 bytes in UTF-8 that is not empty, not `.` or `..`; holds no `/` or `\`, none of `:<>|"?*`, no `~`, no control character (below U+0020) and no lone surrogate; does not end in a dot or a space; and is not a Windows device name: `CON`, `PRN`, `AUX`, `NUL`, `CONIN$`, `CONOUT$`, `COM1` to `COM9` or `LPT1` to `LPT9`, with `COM¹`, `COM²`, `COM³`, `LPT¹`, `LPT²` and `LPT³`, in any case, alone or before a dot, with or without spaces before that dot (`nul.txt`, `nul .txt`). Every path in the manifest MUST be a bare name, and so MUST each chain's path with `.anchors.jsonl` after it, the longer of its sidecars' names. The **names a verifier reads** for a manifest are `manifest.json`, the files of its declared seals, each chain and its two sidecars, and each artifact; no two of them may be equal once folded as §10.4 folds a zip's members, which brings two cases or two Unicode forms of one name together. A verifier reads a name only when a file at the top of the package holds exactly that name, as the folder's listing spells it, and a file whose name folds to a name it reads without being it refuses the package (§10.4). All of this is judged by the names' characters and the folder's listing alone, the same on every system, because a recipient's verdict must not depend on where they verify, and each rule refuses a name one system opens as another file than the rest do: Windows alone reads `C:x` as a drive, `a\b` as a folder, `a?b` as `a_b`, `project.json.` as `project.json` and `NUL` as a device, and opens `LONGFI~1.TXT` for `longfilename.txt` by its 8.3 short name, which no listing shows; Windows and macOS open `PROJECT.JSON` for `project.json`; and Linux and macOS take no name past 255 bytes (#358). Names are expected in Unicode normal form NFC: a file system that lists them in another form, as HFS+ lists NFD, holds another spelling of each non-ASCII name, and the package is refused there. A verifier opens only the files §10 names, each at the top of the package, and never follows a name out of it. A symbolic link inside a folder package is followed like any other file, wherever it points, as §3 says of file references: the rule is about the names, not about containment.
...
| `chains` | array | One row per chain, at least one. Each row MUST hold `path`, a bare name; `head`, a string, the chain head; and `entries`, an integer (never a boolean), the chain's line count. It MAY hold `anchors` and `stamps`, the chain's sidecar names or `null`, and `transcript`, only when a transcript travels with the chain's session. |
| `artifacts` | array | One row per file written after the chains close, possibly none. Each row MUST hold `path`, a bare name; `sha256`, a string, the lowercase hex SHA256 of the file's bytes; and `bytes`, an integer (never a boolean), its length. |
...
- **An authority timestamp's token**, the manifest's and each chain's, goes to `openssl ts -verify`, with the SHA256 it must carry and the certificate chain the recipient saved (ADR-0032). The verifier reads nothing in a token itself, only the status of the reply around it and whether a token follows (§9.7 step 4). When `openssl` refuses a token because a certificate in the chain has expired, the verifier asks again as of the time the token states, which `openssl ts -reply -token_out -text` prints (#264): accepted then, the token is not judged, since past its certificate nothing vouches for the key, and refused then, or with no time that can be read, it is `SEAL-INVALID` (for a chain's token, `STAMP-INVALID`).
...
The conformance vectors hold the package too: [`tests/vectors/`](../tests/vectors/README.md) has an unsealed package, an altered artifact, a zip naming one file twice, a declared seal missing, an unknown format, an anchored manifest, six names that are not bare, `C:x`, `a\b`, `project.json.`, `NUL`, `a?b` and `LONGFI~1.TXT`, a file held as `PROJECT.JSON` for a listed `project.json`, and two listed names that fold to one, each with its exit and its verdict.
