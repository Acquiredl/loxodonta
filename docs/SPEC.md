...
- That rule is about the path's spelling, not containment: nothing checks where a path leads. A symbolic link under the reference base is followed like any other path, so the reference carries the link's `path`, and `sha256` is of the target's bytes at the moment of logging, wherever the target sits. `--files` (§6) follows the link the same way, on the verifying machine. *(Stated 2026-09-18, #224; behavior since v0.1, not a format change: v0.1 hashes are unaffected.)*
