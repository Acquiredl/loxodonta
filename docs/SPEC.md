...
A sidecar's name may hold something that is not a regular file once links are followed: a folder, a pipe, a device, or a file the verifying user may not open. The writer can put one there with one command, so it is judged as a line that cannot be read, never as an absent sidecar and never as a reason to stop (#364). `verify --anchors` prints `ANCHOR-INVALID: <sidecar> cannot be read as a sidecar: <why>`, with `<sidecar>` the bare name and `<why>` as `it is a folder, not a file`, and `verify --stamps` prints the same after `STAMP-INVALID`; each is exit 3, as an unreadable line is (§9.2). In a package, one beside a chain is that chain's finding, `ANCHOR-MISMATCH` or `STAMP-INVALID` in the package's words (§10.5), and the artifact it stands in for is `ARTIFACT-DIVERGED`; one beside the manifest, under a declared seal, is `SEAL-INVALID` (§10.6). A reader asks what the name holds before it opens it, since opening a pipe waits for a writer. The recorder's verbs that append to one say why they could not and exit 73 (`anchor`, `stamp`, `publish`), or 66 for `anchor --upgrade`, which must read it first; the session end passes it by quietly, as it does every failure.
