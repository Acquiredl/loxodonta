...
A **bare name** is a string that is not empty, not `.` or `..`, holds no `/`, no `\`, no `:` and no control character (below U+0020), does not end in a dot or a space, and is not a Windows device name: `CON`, `PRN`, `AUX`, `NUL`, `COM1` to `COM9` or `LPT1` to `LPT9`, with `COM¹`, `COM²`, `COM³`, `LPT¹`, `LPT²` and `LPT³`, in any case, alone or before a dot (`nul.txt`). It is judged by its characters alone, the same on every system, because a recipient's verdict must not depend on where they verify: Windows alone reads `C:x` as a drive, `a\b` as a folder, `project.json.` as `project.json` and `NUL` as a device, and each is refused everywhere (#358). Every path in the manifest MUST be a bare name. A verifier opens only the files §10 names, each at the top of the package, and never follows a name out of it. A symbolic link inside a folder package is followed like any other file, wherever it points, as §3 says of file references: the rule is about the names, not about containment.
