...
- A *torn tail* is the honest way a log gets damaged: the final line left partial. Two causes, both innocent — a crash mid-append, or two writers appending at once (ADR-0004). Verify reports it distinctly (entries before it are intact; operator trims the partial line by hand) but it is still `BROKEN`/exit 1. A malformed line anywhere *else* has no innocent explanation and reports as ordinary tampering. A *forked tail* is the second shape of tail damage (ADR-0004 addendum): a well-formed final entry whose `n` is not its line number, two entries claiming one `n`, which a lock taken from a paused writer leaves behind. It cannot be extended either, so that the fork stays at the tail where innocent damage lives.
