...
The set of tool calls that owe a receipt: defined by the matchers wired into the harness settings, `PostToolUse` for a completed call and `PostToolUseFailure` for one that ran and failed (#239), *as of a moment in time* (ADR-0016 widened the shipped default to `*` — every completed tool call). Coverage is not [completeness](#completeness): coverage says which calls *owe*; the witness judges whether the owed receipts *arrived* — and it judges each session by the coverage in force at that session's time, never by today's rules. A denied tool call, or one the harness rejects before it runs, sits outside coverage by harness design: no hook fires, so no receipt is owed. So does anything the harness does on the session's behalf outside a tool call — a worktree it merges or prunes, a context compaction, its own git plumbing — because no tool event exists to hook (#156): git's reflog is the record for a repository pointer that moved that way, and the chain never claims it. The transcript marks a failed call and a denied one alike, so where the failed-call event is wired the witness owes a failed shell command its receipt, whose result says it ran, and counts any other failed call as one that may be owed: its receipt is not surplus, and its absence is not a deficit. The effective dating has an edge of its own: [calibration](#calibration) starts when the supervisor first looked, and a session whose first tool event is older than that is [before memory](#before-memory) rather than judged by today's rules (ADR-0029).
