...
The supervisor's effective-dated memory of which [coverage](#coverage) was wired when (ADR-0016 ruling 2): a list of observations, each stamping the moment it was made and the matchers seen then, so the witness judges every session by the rules in force at that session's time and a matcher change manufactures no scars. It lives in the baseline — writer-reachable, trusted for nothing beyond calibration — and it has an inception, because an observation is a thing the supervisor did at a moment rather than a fact about all time. What lies before that inception is [before memory](#before-memory). An operator who knows what was wired earlier may seed an epoch ahead of the first observation (`supervisor calibrate --since`); such an epoch is marked as operator-stated and never overwrites observed time, on the git-graft principle that asserted history is allowed and is visible.
